...
const MockKoffi = {
    ...
    // Array definition
    array: (type: any, len: number, hint?: string) => ({ type, len, hint, name: `[${len}]` } as IKoffiCType),

    // Sizeof - return fixed sizes for known structs/types
    sizeof: (type: any) => {
        if (typeof type === 'string') {
            switch(type) {
                case 'uint64': case 'int64': case 'double': return 8;
                case 'uint32': case 'int32': case 'float': return 4;
                case 'uint16': case 'int16': return 2;
                case 'uint8': case 'int8': return 1;
                default: return 8; // Pointer size assumed
            }
        }
